...
Canonical name table entry for GP_HPGe20.

## 2. Detector description

Detector type: General-purpose coaxial HPGe spectrometer 20% relative efficiency.

Crystal/material hint: Coaxial HPGe 20% relative efficiency (UNVERIFIED — requires confirmation against detector passport).
...
LSRM path hint for reference only: Work\GP\HPGe(20%)\Spe\  (operator-side only, F-115)

## 8. Isolation policy (F-83)

This folder is isolated from other detectors. Algorithms in scripts/gamma/ are shared, but data assets here are valid only for the GP_HPGe20 class.
