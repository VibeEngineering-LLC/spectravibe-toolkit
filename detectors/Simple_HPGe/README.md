...
|---------------|------------------|------------|
| LeGe/planar HPGe (demo unit) | Simple_HPGe | TBD |

State: "Crystal-class source pin: audit/_rag/visual_templates/SCHEMA.md:10"

Canonical name table entry for Simple_HPGe.

## 2. Detector description

Detector type: Simple/demo planar HPGe spectrometer

Crystal/material hint: LeGe/planar HPGe (demo unit, UNVERIFIED — requires confirmation against detector passport)
...
LSRM path hint for reference only: Work\Simple\HPGe(Demo)\SPE\10cm\  (operator-side only, F-115)

## 8. Isolation policy (F-83)

This folder is isolated from other detectors. Algorithms in scripts/gamma/ are shared, but data assets here are valid only for the Simple_HPGe class.
