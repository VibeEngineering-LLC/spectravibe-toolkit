# Handy_NaI Detector Class

## 1. Taxonomy lock — Crystal vs Station (2026-06-05)

| Crystal-class | Station-instance | Resolution |
|---------------|------------------|------------|
| NaI(Tl) 2x2 inch or 3x3 inch (UNVERIFIED — requires confirmation against detector passport) | Handy_NaI | TBD |

Canonical name table entry for Handy_NaI: ✅ Confirmed

## 2. Detector description

This is a handheld NaI(Tl) scintillation spectrometer.

«UNVERIFIED — requires confirmation against detector passport»

Crystal material: Sodium Iodide doped with Thallium (NaI(Tl))
...
LSRM path hint for reference only: Work\Handy\Handy(NaI)\Spe\  (operator-side only, F-115)

## 8. Isolation policy (F-83)

This folder is isolated from other detectors. Algorithms in scripts/gamma/ are shared, but data assets here are valid only for the Handy_NaI class.

## 11. Cross-refs

F-83, F-115, F-150, F-153, F-155, F-256, F-265, detector taxonomy lock 2026-06-05.

Note: SPECTRA_INDEX records for this class: 32 (source: audit/_plans/F-300_W0_recon_report.md §2).
